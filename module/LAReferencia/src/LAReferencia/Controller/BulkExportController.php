...
use Exception;
use LAReferencia\BulkExportConfirm;
...
class BulkExportController extends \VuFind\Controller\AbstractBase
{
        ...
        if ($form->isValid()) {
            ...
            if ($showOptionalFields) {
                $selectFields = explode(
                    ',',
                    $exportConfig->Query->selectFields,
                );
            ...
            $useForeignAbstract =
                $exportConfig->Query->showForeignAbstractOption;
            ...
            if ($primaryAbstract == 'yes') {
                array_push(
                    $fullFieldList,
                    $exportConfig->Query->primaryLangAbstract,
                );
                $hasAbstract = 'true';
            }
            ...
            if ($useForeignAbstract) {
                ...
                if ($foreignAbstract == 'yes') {
                    array_push(
                        $fullFieldList,
                        $exportConfig->Query->foreignLangAbstract,
                    );
                    $hasAbstract = 'true';
                }
            }
            ...
            if ($totalRecords <= $maxTotal or $fileExists == 'true') {
                ...
                $response = $this->callExportService(
                    $serviceUrl,
                    $paramString,
                    $type,
                    $totalRecords,
                ...
                $msg = [
                    ...
                    'html' => true,
                    'msg' => $this->getViewRenderer()->render(
                        'bulkexport/captcha-success.phtml',
                        $params,
                    ),
                ];
                $this->flashMessenger()->addMessage($msg, 'success');
            ...
            $msg = [
                ...
                'html' => true,
                'msg' => $this->getViewRenderer()->render(
                    'bulkexport/captcha-error.phtml',
                    $params,
                ),
            ];
    ...
    protected function getParamString()
    {
        ...
        $searchClassId = $this->params()->fromQuery(
            'searchClassId',
    ...
    protected function getQueryBuilder()
    {
        ...
        $defaultDismax = isset($mainConfig->Index->default_dismax_handler)
            ? $mainConfig->Index->default_dismax_handler
            : 'dismax';
        $builder = new QueryBuilder($specs, $defaultDismax);

        //Configure builder
        $searchConfig = $this->getConf($this->searchConf);
        $caseSensitiveBooleans = isset(
            $searchConfig->General->case_sensitive_bools,
        )
            ? $searchConfig->General->case_sensitive_bools
            : true;
        $caseSensitiveRanges = isset(
            $searchConfig->General->case_sensitive_ranges,
        )
            ? $searchConfig->General->case_sensitive_ranges
            : true;
    ...
    protected function injectSpellingParams(ParamBag $params)
    {
        ...
        if ($mainConfig->Spelling->enabled ?? true) {
            $dictionaries =
                $mainConfig->Spelling->simple ?? false
                ? ['basicSpell']
                : ['default', 'basicSpell'];
    ...
    protected function injectConditionalFilter(ParamBag $params)
    {
        ...
        if (
            isset($searchConfig->ConditionalHiddenFilters) &&
            $searchConfig->ConditionalHiddenFilters->count() > 0
        ) {
    ...
    protected function callExportService(
        $serviceUrl,
        $paramString,
        $type,
        $totalRecords,
