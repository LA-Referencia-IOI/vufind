...
namespace VuFindTest\Backend\SemanticSearch {
    ...
    class BackendTest extends TestCase
    {
        ...
        public function testRawJsonSearchWithoutEmbeddingFallsBack(): void
        {
            ...
            $backend = new Backend($connector, $embeddingService, 'my_vector', 0.7);
            $backend->setQueryBuilder($queryBuilder);
            $this->assertEquals('{}', $backend->rawJsonSearch(new Query('hello world'), 5, 10));
        }

        /**
         * Test getEmbedding removal - checking if using the service works.
         *
         * @return void
         */
        public function testEmbeddingServiceIntegration(): void
        {
            $connector = $this->createMock(Connector::class);
            $connector->expects($this->once())->method('search')->willReturn('{}');
            $embeddingService = $this->createMock(EmbeddingService::class);
            $embeddingService->method('embed')->willThrowException(new \Exception('network error'));
            $backend = new Backend($connector, $embeddingService, 'my_vector', 0.7);
            $queryBuilder = $this->createMock(QueryBuilder::class);
            $queryBuilder->method('build')->willReturn(new ParamBag(['q' => 'foo']));
            $backend->setQueryBuilder($queryBuilder);
            $this->assertEquals('{}', $backend->rawJsonSearch(new Query('hello'), 0, 10));
        }
    }
}
